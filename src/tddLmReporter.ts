/**
 * TDD-LM-Reporter for Playwright (custom HTML reporter)
 * @author Lahiru Madhawa
 * @description Real-time HTML reporter with steps, screenshots, videos, and traces
 * @version 1.0.0
 */
...
import { renderAdvancedHtml } from './advancedHtml';
import { writeAnalyticsArtifacts } from './analyticsExporter';
import { isCiEnvironment, JenkinsRunManifest, writeJenkinsArtifacts } from './jenkinsArtifacts';
import { buildStepTree, flattenSteps } from './stepTree';
import {
    getHtmlThemeAttribute,
    getSimpleDarkComponentOverrides,
    getSimpleThemeVariables,
} from './reportThemes';
import {
    ReportRenderContext,
    ...
    StepData,
    SuiteStats,
    TddLmReporterOptions,
    TestData,
} from './types';

export type { TddLmReporterOptions, ReporterMode, ReportTheme } from './types';

class TddLmReporter implements Reporter {
    private readonly options: ResolvedReporterOptions;
    private testResults: TestData[] = [];
    private fileGroups: Map<string, { file: string; describes: Map<string, TestData[]>; stats: { passed: number; failed: number; skipped: number; total: number } }> = new Map();
    ...
    private lastRunManifest?: JenkinsRunManifest;

    constructor(options: TddLmReporterOptions = {}) {
        this.options = {
            ...
            authorVariable: options.authorVariable ?? 'TEST_AUTHOR',
            defaultAuthor: options.defaultAuthor ?? 'QA',
            liveRefreshSeconds: options.liveRefreshSeconds ?? (isCiEnvironment() ? 0 : 5),
            analyticsDir: options.analyticsDir ?? '.analytics',
    ...
    private getEnvironmentLabel(): string {
        return process.env[this.options.environmentVariable] || this.options.defaultEnvironment;
    }

    private getAuthorLabel(): string {
        return process.env[this.options.authorVariable] || this.options.defaultAuthor;
    }
    ...
    onBegin(config: FullConfig, suite: Suite): void {
        ...
        this.outputFile = path.join(this.outputDir, `report_${this.runId}.html`);
        this.config = config;
        ...
        console.log(`║  🌐 Environment: ${environment.padEnd(42)}║`);
        console.log('╚══════════════════════════════════════════════════════════════╝\n');
    ...
    onTestEnd(test: TestCase, result: TestResult): void {
        ...
        const testStartTime = this.testStartTimeMap.get(test.id) || Date.now();
        const stepTree = buildStepTree(result.steps, testStartTime);
        const currentTestSteps = flattenSteps(stepTree);
        ...
        for (const attachment of result.attachments) {
            if (attachment.contentType === 'image/png') {
                ...
                const destPath = path.join(this.outputDir, 'screenshots', screenshotName);
            ...
            if (attachment.contentType === 'video/webm' && attachment.path) {
                // Prefer main test page recording; skip auxiliary page videos (e.g. axe finishRun blank page).
                if (!/[\\/]video\.webm$/i.test(attachment.path)) {
                    continue;
                }
                const videoName = `video_${this.testCounter}.webm`;
                const destPath = path.join(this.outputDir, 'videos', videoName);
                const destDir = path.dirname(destPath);
                if (!fs.existsSync(destDir)) {
                    fs.mkdirSync(destDir, { recursive: true });
                }
            ...
            if (attachment.name === 'trace' && attachment.path) {
                ...
                const destPath = path.join(this.outputDir, 'traces', traceName);
                const destDir = path.dirname(destPath);
                if (!fs.existsSync(destDir)) {
                    fs.mkdirSync(destDir, { recursive: true });
                }
        ...
        const tagSource = [test.title, ...describePath].join(' ');
        const tagMatches = [...new Set(tagSource.match(/@[\w-]+/g) || [])];
    ...
    private generateHTMLRealTime(): string {
        ...
        html = html.replace(
            '<meta charset="UTF-8">',
            this.options.liveRefreshSeconds > 0
                ? `<meta charset="UTF-8">\n    <meta http-equiv="refresh" content="${this.options.liveRefreshSeconds}">`
                : '<meta charset="UTF-8">',
        );
    ...
    async onEnd(_result: FullResult): Promise<void> {
        ...
        console.log('╠══════════════════════════════════════════════════════════════╣');
        console.log(`║  ✅ Passed:    ${String(this.suiteStats.passed).padEnd(44)}║`);
        console.log(`║  ❌ Failed:    ${String(this.suiteStats.failed).padEnd(44)}║`);
        console.log(`║  ⏭️  Skipped:  ${String(this.suiteStats.skipped).padEnd(43)}║`);
        console.log(`║  📊 Total:     ${String(this.suiteStats.total).padEnd(44)}║`);
        console.log('╠══════════════════════════════════════════════════════════════╣');
        console.log(`║  ⏱️  Duration: ${duration.padEnd(44)}║`);
        ...
        if (this.lastRunManifest) {
            const reportDir = path.dirname(this.lastRunManifest.reportHtml);
            console.log(`📊 Jenkins HTML report: ${this.lastRunManifest.reportHtml}`);
            console.log(`📋 Latest report copy: ${this.lastRunManifest.latestHtml}`);
            console.log(`📧 Email summary (text): ${this.lastRunManifest.emailSummaryText}`);
            console.log(`📧 Email summary (HTML): ${this.lastRunManifest.emailSummaryHtml}`);
            console.log(`📄 Run manifest: ${path.join(reportDir, 'latest-run.json')}`);
        }
    }
    ...
    private async generateReport(): Promise<void> {
        ...
        writeAnalyticsArtifacts(
            ...
            reportDir,
            this.outputFile,
            this.options.analyticsDir,
        );

        this.lastRunManifest = writeJenkinsArtifacts(this.createRenderContext(), reportDir, this.outputFile);
...
<body>
    ...
    private createRenderContext(): ReportRenderContext {
        ...
        return {
            ...
            browserName,
            platform,
            liveRefreshSeconds: this.options.liveRefreshSeconds,
        };
    }

    private generateHTML(): string {
        if (this.options.mode === 'advanced') {
            return renderAdvancedHtml(this.createRenderContext());
        }
        ...
        return `<!DOCTYPE html>
<html lang="en" ${getHtmlThemeAttribute(this.options.theme)}>
<head>
    <meta charset="UTF-8">
...
<body>
    <div class="header">
        <h1>🎭 ${this.escapeHtml(this.options.reportTitle)}</h1>
        <p class="header-subtitle">${this.escapeHtml(this.options.reportSubtitle)}</p>
    </div>
    ...
    private generateTestTable(): string {
        ...
        for (const test of this.testResults) {
            ...
            const tagsData = [...test.tags, test.title, ...test.describePath, test.fullTitle].join(' ').toLowerCase();

            const testGroup = test.tags.find(t => t.includes('P0') || t.includes('P1') || t.includes('P2')) ||
                              test.describePath[0] || 'E2E';

            const author = this.getAuthorLabel();
    ...
    private renderSimpleStepNodes(test: TestData, steps: StepData[], depth = 0, prefix = ''): string {
        let html = '';

        steps.forEach((step, stepIndex) => {
            const stepIcon = step.status === 'passed' ? '✓' : step.status === 'failed' ? '✗' : '○';
            const stepClass = step.status;
            const stepDomId = `${prefix}${stepIndex}`;
            const stepId = `step-${test.id}-${stepDomId}`;
            const paddingStyle = depth > 0 ? ` style="margin-left:${depth * 18}px"` : '';

            html += `
                <div class="step-item-container"${paddingStyle}>
                    <div class="step-item ${stepClass} expandable" onclick="toggleStepDetails(this, '${stepId}-details')">
                        ...
                        <span class="step-category">${this.escapeHtml(step.category)}</span>
            ...
            if (step.children?.length) {
                html += `<div class="step-children">${this.renderSimpleStepNodes(test, step.children, depth + 1, `${stepDomId}-`)}</div>`;
            }
    ...
    private generateTestDetailPanel(test: TestData): string {
        ...
        if (test.steps.length > 0) {
            ...
            <div class="detail-section steps-section">
                ...
                <div class="section-content">
                    <div class="steps-list">${this.renderSimpleStepNodes(test, test.steps)}</div>
                </div>
            </div>`;
        }
...
export default TddLmReporter;
export { TddLmReporter };
